...
export default function ItemPage({ params }: Params) {
  ...
  const similar = Database.getProducts(product.category).filter(
    (p) => p.id !== product.id,
  );

  const topProducts = Database.getTopProducts(4).filter(
    (p) => p.id !== product.id,
  );
  ...
  return (
    <div>
      <div className="flex gap-2 align-center text-sm mt-1 text-slate-500 pb-5">
        <Link href={"/items"} className="hover:text-slate-700 duration-300">
          Todos los productos
        </Link>
        <div>&gt;</div>
        <Link
          href={"/items?search=" + encodeURI(product.category)}
      ...
      <div className="flex gap-[30px] items-start flex-col md:flex-row">
        ...
        <div className="md:w-2/4 w-full">
          ...
          <div className="w-full flex items-start gap-2 items-center">
            <div className="text-md text-slate-800 truncate max-w-[70%]">
              {product.brand}
            </div>
          ...
          <div className="mt-2">
            The {product.title} is a great product from {product.brand} that
            competes with other products in the{" "}
            {categoryToHuman(product.category)} category.
          </div>
          <div className="mt-2">
            At a price of just {product.price}€, this product offers a really
            good quality that you do not find in other similar products from
            other brands.
          </div>
          <div className="mt-2">
            Like all the products in Decshop, the {product.title} has a
            guarantee of at least 2 years.
          </div>
        </div>
          ...
          <div className="border rounded-md p-5">
            {product.discountPercentage && (
              <div className="flex gap-3">
                ...
                <div className="text-xs font-medium bg-amber-300 rounded-md p-1 pl-2 pr-2 mt-[-3px]">
                  -{product.discountPercentage}%
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col gap-3 mt-5">
              ...
              <Button className="bg-orange-400 hover:bg-orange-500 duration-300 text-black-900">
                Comprar ya
              </Button>
            </div>
          </div>
      ...
      {similar.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {similar.slice(0, 4).map((product, key) => (
              <ProductTile key={key} product={product} />
            ))}
          </div>
        </>
      )}
      ...
      {similar.length === 0 && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {topProducts.slice(0, 4).map((product, key) => (
              <ProductTile key={key} product={product} />
            ))}
          </div>
        </>
